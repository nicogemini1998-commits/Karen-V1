...
echo -e "\n${BOLD}4b. Security hooks v2.2${NC}"
check "hook integrity-ledger.sh ejecutable" "[ -x '$KAREN_CONFIG_DIR/hooks/integrity-ledger.sh' ]"
check "hook mcp-pin-verify.sh ejecutable" "[ -x '$KAREN_CONFIG_DIR/hooks/mcp-pin-verify.sh' ]"
...
check "integrity/ existe" "[ -d '$KAREN_CONFIG_DIR/integrity' ]"
check "backups/ existe" "[ -d '$KAREN_CONFIG_DIR/backups' ]"
check "redteam/ existe" "[ -d '$KAREN_CONFIG_DIR/redteam' ]"
check "quarantine/ existe" "[ -d '$KAREN_CONFIG_DIR/quarantine' ]"

echo -e "\n${BOLD}4e. Firewall rules${NC}"
